...
#' @param prop.iso Proportion of individuals in iso boxes, in this first implementation only two kinds of housing units 
...
#' @param d.rat departure rate (includes external deaths and actual departures) -- if set to 0 no invidual is consider to leave
...
#'  their tent)
#' @param act.rate.se Act rate (contact rate) between susceptible and exposed individuals -- default: 10 interactions/timestep 
...
#' @param act.rate.si Act rate (contact rate) between susceptible and infected individuals -- default: 10 interactions/timestep 
...
#' @param act.rate.sq Act rate (contact rate) between susceptible and quarantined individuals -- default: 2.5 interactions/timestep 
...
net_simulate = function(
  ...
  # Network diagnostics
  print('Getting network diagnostics')
  dx <- netdx(est,
              nsims = 1e3,
              nsteps = 90,
              ncores = cores,
  ...
  param = param.net(act.rate.se = act.rate.se,
  ...
  control = control.net(
    ...
    ncores = cores,
    initialize.FUN = CampNetworkSimulate::custom.initialize.net, # this bit is just so that I can extract time
    exposure.FUN = CampNetworkSimulate::exposure,
    infect.FUN = CampNetworkSimulate::infect,
    epi.by = "age",
    quarantine.FUN = CampNetworkSimulate::quarantining,
    hospitalize.FUN = CampNetworkSimulate::RequireHospitalization,
    recover.FUN = CampNetworkSimulate::recover,
    fatality.FUN = CampNetworkSimulate::fatality,
    recovery.FUN = NULL,
    infection.FUN = NULL,
    departures.FUN = CampNetworkSimulate::departures.net,
    get_prev.FUN = CampNetworkSimulate::custom.get_prev.net,
    skip_check = FALSE,
    depend = T
  )
  ...
  t0 = Sys.time()
  sim = CampNetworkSimulate::custom.netsim(est, param, init, control)
